...
## DMI lightning observations (deferred milestone)

Checked 2026-09-23 against the [DMI lightning API](https://www.dmi.dk/friedata/dokumentation/lightning-data-api), [data description](https://www.dmi.dk/friedata/dokumentation/lightning-data), [authentication notice](https://www.dmi.dk/friedata/dokumentation/authentication), and [terms](https://www.dmi.dk/friedata/dokumentation/terms-of-use). The public OGC Features endpoint supplies triangulated strokes as GeoJSON points with WGS84 coordinates, a UTC `properties.observed` occurrence time, and type 0/1 ground or type 2 cloud. No account or API key is needed. Data is CC BY 4.0, so the UI credits DMI, links the [license](https://creativecommons.org/licenses/by/4.0/) and DMI terms, and states that points are filtered by viewport and age. DMI estimates position accuracy at 500–2,000 m and warns that raw observations can contain errors.

DMI officially describes coverage as **Denmark and surrounding areas**, not all Poland. Direct queries on 2026-09-23 returned actual points in 18–19°E/54–55°N near Gdynia, 20–22°E/52–53°N near Warsaw, and 19–21°E/49–51°N in southern Poland during the previous month. This demonstrates some Polish observations, not complete or continuous detection coverage. The app says so and never interprets an empty response as no lightning. `npm.cmd run smoke:lightning` checked three actual Gdynia-area points; newest occurrence was 2026-09-05T23:24:54.363Z at the time of the check. That historical sample is verification evidence, not displayed as live data.

The app requests only the visible bbox when the user enables or refreshes the layer, limited to 5° on either axis, the latest hour, and 500 points. Hitting the 500-point page limit signals possible truncation; the user can zoom in. DMI supplied a `next` link even for a 12-point page whose successor was empty, so that link alone is not used as proof of missing observations. No automatic polling, background requests, bulk download, persistent strike history or private feed access. Fetches are cancelled when the app backgrounds, the layer is hidden, or connectivity is lost. Returned data remains only in memory during this app session and ages out of the map at 60 minutes. On resume, the user can refresh explicitly. The UI distinguishes occurrence time from last successful fetch, notes stale/failed/offline responses, and provides 10/30/60-minute filters. This bounded, user-triggered use follows DMI's mobile and fair-use guidance. Service timeliness and retention are not guaranteed by DMI; the app makes no alert or safety claim.

Blitzortung's public map is not a licensed raw point API; its [participant documentation](https://www.blitzortung.org/en/compendium.php) restricts raw-data access. EUMETSAT offers [free satellite lightning flashes](https://user.eumetsat.int/catalogue/EO%3AEUM%3ADAT%3A0691), but its Data Store requires [registration](https://user.eumetsat.int/resources/user-guides/data-registration-and-licensing) and netCDF processing; satellite flashes are also a different measurement from ground strokes. Neither source is silently substituted for DMI observations.
